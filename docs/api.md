...
| 전역 prefix | 모든 API는 `/api`로 시작한다. 예외: 링크 웹 페이지 `GET /t/{token}` (⏳) |
...
| `drawer.stored` | 보관 중인 테이프 수(분류 안 함 + 모든 칸). ⏳ 2단계 전까지 항상 0 |
| `drawer.full` | `stored >= cap`. 서랍 꽉 참 배너(`fullOn`) |
| `tapes` | 보유 테이프. 1분은 무제한이라 `qty: null`("무료") |
| `stats.receivedCount` | 받은 테이프 수 = 보관량 (디자인과 같음). ⏳ 2단계 전까지 0 |
| `stats.sentCount` | 보낸 테이프 수. ⏳ 2단계 전까지 0 |
...
녹음 파일은 앱이 MinIO에 **직접** 올린다(presigned PUT). 형식은 AAC(m4a) 64kbps, 최대 6MB.
...
```
- `durationMs ≤ tapeType × 60,000 + 1,000`. 넘으면 `400 RECORDING_TOO_LONG`
...
업로드가 끝나면 부른다. 서버가 파일을 확인하고 변환 큐(ffmpeg "테이프 소리")에 넣는다. 응답 `200 Recording` (`status: "processing"`)

### ⏳ `GET /recordings/{id}`
확인 화면(`vConfirm`)이 1초 간격으로 부른다. "테이프 소리로 바꾸는 중…"은 `ready`가 될 때까지, **최소 1.4초** 보여 준다. 1.4초를 넘기면 대기 표현(`convSlowOn`).
```json
{
  "id": "…",
  "tapeType": 3,
...
| `failed` | 변환 실패(`convFailOn`): 다시 시도 → `POST /recordings/{id}/retry` · 처음부터 다시 녹음 |

`preview`는 **보내기 전의 녹음 주인**에게만 준다. 보낸 뒤에는 `null`.

### ⏳ `POST /recordings/{id}/retry`
`failed`인 녹음을 다시 변환한다. 응답 `200 Recording`
...
### ⏳ `POST /deliveries` 🔑
라벨 화면(`vLabel`)의 "보내기". 한 트랜잭션에서 **보유 테이프 차감(3·5분만) → 테이프 생성 → 친구 `lastAt` 갱신**, 끝나면 받는 사람에게 푸시.
...
- 받는 사람 서랍이 꽉 차도 보낸다("분류 안 함"에 들어가고, 받는 쪽 앱이 배너를 띄운다)
- 받는 사람이 나를 차단했어도 `201`로 보이게 한다(받는 쪽에는 들어가지 않는다)

응답 `201 SentTape` (링크면 `share.url`로 카카오톡/문자 공유 시트를 연다)
오류 `409 NO_TAPE_LEFT`, `409 RECORDING_NOT_READY`, `409 RECORDING_ALREADY_SENT`, `403 NOT_FRIEND`, `404 RECORDING_NOT_FOUND`
보내기 실패(`sendFailOn`) → "다시 보내기"는 **같은 `Idempotency-Key`**로 재시도한다.

### ⏳ `GET /deliveries/sent?cursor=&limit=`
마이 > 보낸 테이프. 최근 순. `{ "items": [SentTape], "nextCursor": null }`

### ⏳ `GET /deliveries/sent/{id}`
보낸 테이프 상세(`shSentDetail`). `200 SentTape`

### ⏳ `POST /deliveries/sent/{id}/share`
"링크 다시 공유하기". 아직 안 받은 링크 테이프만. 만료 전이면 같은 링크를, 만료됐으면 새 링크(7일)를 준다.
응답 `200 { "url": "https://<도메인>/t/…", "expiresAt": "…" }` · 받은 뒤면 `409 LINK_TAKEN`

### ⏳ `GET /deliveries/{id}`
받은 테이프 하나(푸시를 눌러 들어올 때). `200 ShelfItem`
...
```
불러오는 중(`vLoadingOn`) → 실패하면 `vErrorOn`("다시 시도"). 오류 `403 FORBIDDEN`, `404 TAPE_NOT_FOUND`, `409 AUDIO_NOT_READY`
...
### ⏳ `POST /shelf/groups`
칸 추가(무료). `{ "name": "2026 생일" }` (1~12자, 비우면 "새 칸") → `201 { "id", "name", "items": [] }`

### ⏳ `PATCH /shelf/groups/{id}`
`{ "name": "새 이름" }` 또는 순서 `{ "afterId": "앞 칸 id" | null }` (null = 맨 앞) → `200 Group`

### ⏳ `DELETE /shelf/groups/{id}`
칸 지우기. 안에 있던 테이프는 "분류 안 함"으로 가고 뜯은 상태가 된다("칸을 지웠어요 · 테이프는 분류 안 함으로"). `204`

### ⏳ `PATCH /shelf/items/{id}`
드래그 정렬, 옮기기 시트(`shMove`).
```json
{ "groupId": "칸 id 또는 null(분류 안 함)", "afterId": "바로 앞 테이프 id 또는 null(맨 앞)" }
```
응답 `200 ShelfItem`. 앱은 낙관적으로 먼저 옮기고, 실패하면 되돌린다. 안 뜯은 소포는 옮길 수 없다(`409`).

### ⏳ `DELETE /shelf/items/{id}`
테이프 지우기(`itemDel`, "테이프를 지웠어요"). 파일도 지운다(보낸 사람도 못 듣기 때문에). `204`
...
### ⏳ `GET /share/{token}`
앱에서 링크를 열었을 때. 받을 수 있으면 소포 화면(`vParcel` + `viaLink` 칩)을 띄울 정보를 준다.
```json
{ "sender": { "userId": "…", "name": "하늘" }, "tapeType": 1, "tag": "thinking", "sentAt": "…", "expiresAt": "…" }
```
| 오류 | 화면(`leOn`) |
|---|---|
| `409 LINK_TAKEN` | 이미 다른 분이 받은 테이프예요 |
| `410 LINK_EXPIRED` | 링크가 만료됐어요 |
| `409 LINK_OWN` (+`deliveryId`, `url`) | 내가 보낸 테이프예요 → 링크 다시 공유하기 |
| `404 LINK_NOT_FOUND` | 찾을 수 없어요 |

### ⏳ `POST /share/{token}/claim` 🔑
"뜯기"를 누르면 받는다. 한 트랜잭션에서 받는 사람을 채우고 **서로 친구**가 된다(보낸 사람이 나를 차단했으면 친구는 맺지 않는다).
응답 `200 { "item": ShelfItem, "friend": Friend }` → 재생 후 "○○님과 친구가 되었어요". 오류는 위 표와 같다.

### ⏳ `GET /share/{token}/web` @공개 · `POST /share/{token}/web/audio` @공개
앱이 없는 사람의 모바일 웹 페이지(`webOn`)용. 미리보기 정보와 재생 URL(10분)을 준다. 웹에서 들어도 받은 것(claim)으로 치지 않는다. 오류는 위 표와 같다(`LINK_OWN`은 없음).

### ⏳ `GET /t/{token}` @공개 (`/api` 밖, HTML)
모바일 웹 페이지: "○○님이 테이프를 보냈어요" → 소포 뜯기 → 웹 재생, 아래에 App Store / Google Play. `apple-app-site-association`, `assetlinks.json`도 같이 제공한다.
...
| 녹음 확인 `vConfirm` `convSlowOn` `convFailOn` | `POST /recordings` → PUT 업로드 → `POST /recordings/{id}/complete` → `GET /recordings/{id}` 폴링 → 실패 시 `POST /recordings/{id}/retry` ⏳ |
...
| 받은 테이프(서랍, 칸)와 그 녹음 파일 | 삭제 | ⏳ 2단계 |
| 보낸 테이프 | **받은 사람의 서랍에는 남긴다**(받은 사람의 것). 보낸 사람은 `sender.userId: null` + 보낼 때 이름으로 보인다. 아직 아무도 안 받은 링크 테이프는 삭제 | ⏳ 2단계 |
| 보내지 않은 녹음 | 삭제 | ⏳ 2단계 |
| FCM 토큰 | 삭제 | ⏳ 2단계 |
